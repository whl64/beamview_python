...
class SettingsWindow(QWidget):
    ...
    def build_stat_frame(self):
        ...
        manual_range_button = QPushButton(text='Set manual range', parent=stat_frame)
        manual_range_button.clicked.connect(self.manual_range)
        manual_range_layout.addWidget(manual_range_button)
        
        range_validator = QIntValidator(self)
        self.min_range_entry = QLineEdit(parent=stat_frame)
        self.max_range_entry = QLineEdit(parent=stat_frame)
        ...
        manual_range_layout.addWidget(QLabel(text='-', parent=stat_frame))
        ...
        save_button = QPushButton(text='Save image', parent=self)
        save_button.clicked.connect(self.save_image)
        
        # self.archive_check = QCheckBox(text='Archive mode?', parent=self)
    ...
    def build_proc_frame(self):
        ...
        proc_row_layout = QHBoxLayout()
        self.main_layout.addLayout(proc_row_layout)
        proc_frame = QGroupBox(title='Image processing controls', parent=self)
        proc_row_layout.addWidget(proc_frame)
        proc_row_layout.addStretch(1)

        proc_layout = QVBoxLayout()
        proc_frame.setLayout(proc_layout)
        self.median_check = QCheckBox(text='Use median filter?', parent=proc_frame)
        self.median_check.clicked.connect(self.median_check_click)
        median_row_layout = QHBoxLayout()
        proc_layout.addLayout(median_row_layout)
        median_row_layout.addWidget(self.median_check)
        median_row_layout.addStretch(1)
        
        thresh_row_layout = QHBoxLayout()
        proc_layout.addLayout(thresh_row_layout)
        self.thresh_check = QCheckBox(text='Use threshold?', parent=proc_frame)
        thresh_row_layout.addWidget(self.thresh_check)
        self.thresh_check.clicked.connect(self.thresh_check_click)      
        thresh_row_layout.addWidget(QLabel(text='Threshold value (%): ', parent=proc_frame))      
        self.thresh_entry = QLineEdit(parent=proc_frame)
        self.thresh_entry.setValidator(QDoubleValidator(bottom=0, top=100, parent=self))
        ...
        colormap_row_layout = QHBoxLayout()
        ...
        colormap_row_layout.addWidget(QLabel(text='Colormap: ', parent=proc_frame))      
    ...
    def build_camera_frame(self):
        ...
        acq_row_layout = QHBoxLayout()
        self.main_layout.addLayout(acq_row_layout)
        acq_frame = QGroupBox(title='Acquisition controls', parent=self)
        acq_row_layout.addWidget(acq_frame)
        acq_row_layout.addStretch(1)
        acq_layout = QGridLayout()
        acq_frame.setLayout(acq_layout)

        acq_layout.addWidget(QLabel(text='Exposure time (ms): ', parent=self), 0, 0, Qt.AlignmentFlag.AlignRight)
        
        dbl_validator = QDoubleValidator(bottom=1e-3, parent=self)
        self.exposure_entry = QLineEdit(parent=acq_frame)
        ...
        acq_layout.addWidget(QLabel(text='Gain: ', parent=self), 1, 0, Qt.AlignmentFlag.AlignRight)
        int_validator = QIntValidator(bottom=0, parent=self)
        self.gain_entry = QLineEdit(parent=acq_frame)
        ...
        self.trigger_check = QCheckBox(parent=self)
        trigger_label = QLabel(text='Trigger on?', parent=self)
        ...
        size_frame = QGroupBox(title='AOI controls', parent=self)
        size_row_layout.addWidget(size_frame)
        size_row_layout.addStretch(1)
        size_layout = QGridLayout()
        size_frame.setLayout(size_layout)
        
        size_layout.addWidget(QLabel(text='x:', parent=size_frame), 0, 0)
        size_layout.addWidget(QLabel(text='-', parent=size_frame), 0, 2)
        size_layout.addWidget(QLabel(text='y:', parent=size_frame), 1, 0)
        size_layout.addWidget(QLabel(text='-', parent=size_frame), 1, 2)
        
        self.reset_size_button = QPushButton(text='Reset', parent=self)
        self.reset_size_button.clicked.connect(self.reset_size)
        size_layout.addWidget(self.reset_size_button, 0, 4, 2, 2)

        self.x_validator = QIntValidator(bottom=0, top=0, parent=self)
        self.y_validator = QIntValidator(bottom=0, top=0, parent=self)
        
        self.min_x_entry = QLineEdit(parent=size_frame)
        self.min_x_entry.setValidator(self.x_validator)
        self.max_x_entry = QLineEdit(parent=size_frame)
        self.max_x_entry.setValidator(self.x_validator)
        self.min_y_entry = QLineEdit(parent=size_frame)
        self.min_y_entry.setValidator(self.y_validator)
        self.max_y_entry = QLineEdit(parent=size_frame)
        ...
        self.calibration_check = QCheckBox(text='Use pixel calibration? (um/px)', parent=self)
        self.calibration_check.clicked.connect(self.calibration_changed)
        calibration_layout.addWidget(self.calibration_check)
        
        calibration_validator = QDoubleValidator(bottom=1e-12, parent=self)
        self.calibration_entry = QLineEdit(parent=self)
